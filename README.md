...
- 首页展示当前账号下的全部文档
- 支持创建、编辑、删除和拖拽排序分类；删除分类时会将其下文档迁移到“默认分类”
- 支持按分类页签筛选文档；在当前分类下新建记忆时会自动预选该分类
- 支持右上角分享给好友 / 朋友圈，未登录用户从朋友圈点开会看到公开介绍页
- 支持按标题和正文内容搜索，标题命中会高亮显示
- 支持左滑删除、批量选择删除、当前列表全选 / 取消全选、空状态引导
- 支持文档列表加载态，避免登录后因云端数据加载延迟闪现空状态
- 首页在短时间内返回时会复用已加载的数据与封面缓存，减少重复请求
- 使用自定义 TabBar 在“家 / 我的”两个主页面之间切换，并优化图标预加载和切换状态

### 3. OCR 建稿与编辑

- 支持从相册或相机选择图片，单次最多 9 张
- 图片会先上传到微信云存储，再创建 OCR 任务
- 支持多图识别、状态轮询、失败提示、稍后刷新结果
- 识别完成后自动生成文档标题、摘要和 Markdown 草稿
- 保存文档前会进行文本与图片内容安全校验，命中违规内容时提示用户修改
- 草稿可继续编辑后保存为正式文档
- 编辑页使用独立滚动区域处理键盘弹出，避免输入标题或正文时页面跳动、内容被遮挡
...
- 支持意见反馈、选填联系方式，并上传图片 / 视频附件；邮件通知会包含联系方式
- 反馈提交前会进行文本与图片内容安全校验
- 支持退出登录
...
- 删除文档时会清理对应文档图片和 OCR 任务记录；注销账号时会清理该账号相关数据库记录、文档图片、反馈附件和自定义头像
